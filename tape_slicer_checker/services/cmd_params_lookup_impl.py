...
class CmdParamsLookupImpl(CmdParamsLookup):
    def __init__(
        self,
        remag_table_lookup: RemagTableLookup,
        remnode_table_lookup: RemnodeTableLookup
    ) -> None:
        self._remag_table_lookup = remag_table_lookup
        self._remnode_table_lookup = remnode_table_lookup
    ...
    def get_params(self, file_name: str) -> CmdParameters:
        ...
        if len(parts) == 2:
            ...
            ag_name, sid = self._remag_table_lookup.agname_sid(agid_name)
            nid: str = self._remnode_table_lookup.nid(sid)
